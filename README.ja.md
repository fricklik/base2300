...
Node.js 22以上（またはブラウザー。ネイティブ圧縮候補には `CompressionStream` が必要）で動作します。
ESM・TypeScript型定義・CLI `base2300` を同梱します。パッケージの版と形式（wire）の版は独立です。
...
書記素数を比較できます。UIは既定で英語、右上の切替（または `?lang=ja`）で日本語になります。処理はWorker内で行い、中止可能です。変換後に元バイトへの完全一致も
確認します。元テキスト・復元結果の上限はデモが64 KiB、ライブラリが4 MiBです。
符号化文字列には、容量の拡大を考慮した別の上限があります。
デモのホスティングはありません。ローカルで実行してください。
